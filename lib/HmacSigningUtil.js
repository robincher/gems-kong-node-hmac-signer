...
let getHmac256Signature = function (signatureString, secret) {
    return crypto.createHmac("sha256", secret).update(signatureString).digest("base64");
}

let getHmac256Digest = function (data) {
...
HmacSigningUtil.getSignatureBaseString = (params) => {
    ...
    if (requestMethod === "POST" || requestMethod === "UPDATE") {
        ...
        bodyDigest = getHmac256Digest(params.data);
    ...
    let signatureHash = getHmac256Signature(signatureString, params.secret);
